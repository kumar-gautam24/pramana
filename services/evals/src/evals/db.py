...
This file intentionally duplicates the same module in the other services rather than
sharing it from packages/common: a migration runner is infrastructure, not a wire
contract, and packages/common is reserved for the latter. This service stores no vectors,
so its one pool needs no codec and serves both migrations and the running application."""
...
# Advisory locks are scoped per-database, and evals never shares one with another
