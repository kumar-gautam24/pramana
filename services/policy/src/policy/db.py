...
async def pool(dsn: str | None = None) -> asyncpg.Pool:
    ...
    Must be called only after migrations have run (use migration_pool() before that).
    The init callback registers the vector codec on every connection this pool opens,
    including the first, and that registration queries pg_type for "vector" -- a query
    that raises on any database where 0001_*.sql (which creates the extension) has not
    run yet. That is the state of every fresh database, so calling pool() before
    migrations is not a corner case, it is the normal bootstrap order getting skipped.
    """
...
async def migration_pool(dsn: str | None = None) -> asyncpg.Pool:
    """A codec-free pool for run_migrations() only. Migrations never read or write a
    vector column, and the extension that defines the type may not exist yet -- true of
    every database before 0001_*.sql has applied. pool()'s init callback needs that
    extension present on every connection it opens, so it cannot be reused here.

    Startup order is: migration_pool() -> run_migrations() -> close it -> pool() for
    the application. By the time pool() opens its first connection, the migration that
    created the extension has already run."""
    return await asyncpg.create_pool(
        dsn or _asyncpg_dsn(get_settings().database_url),
        min_size=0,
    )
