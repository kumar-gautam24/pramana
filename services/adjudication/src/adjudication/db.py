...
This file intentionally duplicates services/policy/src/policy/db.py and
services/member/src/member/db.py rather than sharing them from packages/common: a
migration runner is infrastructure, not a wire contract, and packages/common is reserved
for the latter. This service stores no vectors -- it evaluates criteria against facts
fetched over HTTP from policy and member, not from Postgres -- so, like member, its one
pool needs no vector codec and serves both migrations and the running application."""
...
# Advisory locks are scoped per-database, and adjudication never shares one with policy
# or member, so this literal being reused verbatim in their db.py files cannot cause a
# cross-service collision. Any fixed value works; this one has no meaning beyond being
# constant.
