...
| `packages/common/src/pramana_common/criteria.py` | `CriterionType`, `Verdict`, `Outcome`, `GateReason`, `CriterionResult` — the shared vocabulary |
...
- Produces:
  ...
  - `GateReason` — `StrEnum` with members `NO_CRITERIA`, `CRITERION_NOT_MET`, `INSUFFICIENT_EVIDENCE`, `LOW_CONFIDENCE` (the closed set of things the gate may say to a reviewer)
...
**Interfaces:**
- Consumes: `CriterionResult`, `Verdict`, `Outcome`, `GateReason` from Task 2
- Produces:
  - `GateThresholds(min_confidence: float = 0.0)` — frozen, validated
  - `GateDecision(outcome: Outcome, blocking: tuple[str, ...], reason: GateReason | None)` — frozen
  - `evaluate_gate(results: list[CriterionResult], thresholds: GateThresholds) -> GateDecision`
  - Reasons come from `GateReason`, never from a literal, so the gate cannot invent a fifth thing to say to a reviewer
...
Run: `cd packages/common && uv run pytest tests/test_gate.py -v`
Expected: PASS, 16 tests (the parametrised cases expand).
...
**Interfaces:**
- Consumes: `CriterionType`, `Verdict`, `Outcome`, `GateReason` from Task 2; `GateDecision` from Task 3. The dependency runs one way only: `schemas.py` may import `gate.py`, never the reverse, so the gate stays free of pydantic.
- Produces (pydantic v2 models, used by every service):
  ...
  - `CriterionOutcome(criterion_id: str, verdict: Verdict, confidence: float, tool: str, evidence: tuple[EvidenceSpan, ...])`
  - `CaseRequest(member_id: str, requested_code: str, icd10: str, date_of_service: date, kind: Literal["initial", "continuation"])`
  - `Determination(case_id: int, outcome: Outcome, blocking: tuple[str, ...], reason: GateReason | None, criteria: tuple[CriterionOutcome, ...])`, with a validator coupling `outcome` to `blocking`/`reason`
  - `Determination.from_gate_decision(case_id, decision, criteria)` — the one supported way to put a `GateDecision` on the wire
...
services:
  db:
    ...
    healthcheck:
      ...
      retries: 10
    volumes:
      # The entrypoint runs create-databases.sql only when it initialises an empty data
      # directory, so against a pgdata volume from an earlier run it is skipped in
      # silence -- editing the SQL and restarting looks like it worked and changes
      # nothing. `docker compose down -v` drops the volume so the script runs again.
